...
def main():
    ...
    for i in range(num_round):
        ...
        # LP_order, job_transmission = LP.SWAG_MIP(job_demand, job_duration, job_datasize, capacity, bandwidth, False, "CPLEX")
        # this_cost = time.time() - start_time
        # time_cost[0] += this_cost
        # # print(job_transmission)
        # transmission_list = transfer_transit(LP_order, job_transmission, job_duration, job_datasize, bandwidth)
        # num_transit = sum(transmission_list[i].length() for i in range(num_sites))
        # # print(transmission_list)
        # LP_JRT = compute_JRT(job_demand, job_duration, capacity, LP_order, transmission_list)
        # print(f"CLPEX_LP: {list(map(int, LP_order))}, JRT={LP_JRT}, sum={sum(LP_JRT)}, time cost={this_cost}, transfers={num_transit}")
        # a[2] += sum(LP_JRT)

        # start_time = time.time()
        # LP2_order, job_transmission2 = SWAG_MIP(job_demand2, job_duration, job_datasize, capacity, bandwidth, False, "CPLEX")
        # this_cost = time.time() - start_time
        # time_cost[1] += this_cost
        # # print(job_transmission2)
        # transmission_list2 = transfer_transit(LP2_order, job_transmission2, job_duration, job_datasize, bandwidth)
        # num_transit = sum(transmission_list2[i].length() for i in range(num_sites))
        # LP2_JRT = compute_JRT(job_demand2, job_duration, capacity, LP2_order, transmission_list2)
        # print(f"CPLEX_LP2: {list(map(int, LP2_order))}, JRT={LP2_JRT}, sum={sum(LP2_JRT)}, time cost={this_cost}, transfers={num_transit}")
        # # print(f"{sum(Greedy.compute_JRT(job_demand2, job_duration, capacity, LP_order, transmission_list2))}")
        # a[3] += sum(LP2_JRT)

        # solver2 = "HIGHS"
        slot_workload = [[0] * i for i in capacity]
        start_time = time.time()
        MIP_order, job_transmission3, num_late = max_flow_ortools(job_demand3, job_duration, job_datasize, capacity, bandwidth)
        this_cost = time.time() - start_time
...
def max_flow_ortools(job_demand, job_duration, job_datasize, capacity, bandwidth):  #
    ...
    return max_flow_order, job_transmission, 0
