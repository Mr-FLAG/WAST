                    ...
                    for bandwidth in bandwidths:
                        ...
                        while total_job_queue or total_wait_list:
                            ...
                            if next_job_arrival_time < earliest_event_time:
                                ...
                                while total_job_queue and total_job_queue[0].arrival_time == next_job_arrival_time:
                                    ...
                                    if current_job.num_task == 1:
                                        # Job has 1 task only, add to the highest priority
                            ...
                            if recompute:
                                ...
                                #         temp_index += 1
                                # total_job_in_computation += len(total_wait_list)
...
def compute_order(strategy, total_wait_list, wait_list, capacity, bandwidth, transmission_list, current_time, forecast_threshold=sys.maxsize):
    ...
    if strategy == "SWAG":
        return Policy_SWAG.SWAG_slot(current_demand, current_duration, capacity), transmission_list
    elif strategy == "SRPT":
...
def write_job_completion_time(finish_list, data_resource, strategy, num_of_sites, alpha, bandwidth, utility,
    ...
    with open(path_name, 'w', newline='') as csv_file:
        ...
        while finish_list:
            ...
            num_transmission = finished_job.num_transmission
            distribution = finished_job.distribution

            # Build row with job details
            row = [job_id, num_task, total_duration, arrival_time, finish_time, response_time, num_transmission] + distribution
            writer.writerow(row)
...
def compute_SD(aggregate_allocation):
    length = len(aggregate_allocation)
    if length == 0:
        return 0

    # Compute mean
    mean = sum(aggregate_allocation) / length

    # Compute standard deviation
    SD = sum((i - mean) ** 2 for i in aggregate_allocation)

    return math.sqrt(SD / length)


def record_aggregate_allocation(num_of_sites, wait_list):
    aggregate_allocation = [0] * len(wait_list[0])  # Initialize array with zeros
    for i in range(num_of_sites):
        index = 0
        for temp_job in wait_list[i]:
            aggregate_allocation[index] += temp_job.num_performing_task
            index += 1
    return aggregate_allocation


def compute_deviation(real_aggregate_allocation, allocation):
    num_of_jobs = len(real_aggregate_allocation)
    computed_aggregate_allocation = [0.0] * num_of_jobs

    # Sum up the allocations
    for i in range(len(allocation)):
        for j in range(num_of_jobs):
            computed_aggregate_allocation[j] += allocation[i][j]

    # Compute the difference
    difference = 0
    for j in range(num_of_jobs):
        difference += abs(real_aggregate_allocation[j] - computed_aggregate_allocation[j])
